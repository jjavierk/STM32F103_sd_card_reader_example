...
)
{
	...
	TM_SPI_ReadMulti(SPI_PORT, buff, 0xFF, btr);
}
...
)
{
	...
	TM_SPI_WriteMulti(SPI_PORT, (uint8_t *)buff, btx);
}
#endif
...
{
	FATFS_CS_HIGH;			/* CS = H */
	TM_SPI_Send(SPI_PORT, 0xFF);			/* Dummy clock (force DO hi-z for multiple slave SPI) */
...
static int select_t (void)	/* 1:OK, 0:Timeout */
{
	FATFS_CS_LOW;
	TM_SPI_Send(SPI_PORT, 0xFF);	/* Dummy clock (force DO enabled) */
...
)
{
	...
	do {							// Wait for DataStart token in timeout of 200ms 
		token = TM_SPI_Send(SPI_PORT, 0xFF);
	...
	TM_SPI_Send(SPI_PORT, 0xFF); TM_SPI_Send(SPI_PORT, 0xFF);			// Discard CRC
...
)
{
	...
	TM_SPI_Send(SPI_PORT, token);					/* Send token */
	if (token != 0xFD) {				/* Send data if token is other than StopTran */
		xmit_spi_multi(buff, 512);		/* Data */
		TM_SPI_Send(SPI_PORT, 0xFF); TM_SPI_Send(SPI_PORT, 0xFF);	/* Dummy CRC */

		resp = TM_SPI_Send(SPI_PORT, 0xFF);				/* Receive data resp */
...
)
{
	...
	if (cmd != CMD12) {
		deselect();
		if (!select_t()) return 0xFF;
	}

	/* Send command packet */
	TM_SPI_Send(SPI_PORT, 0x40 | cmd);				/* Start + command index */
	TM_SPI_Send(SPI_PORT, (BYTE)(arg >> 24));		/* Argument[31..24] */
	TM_SPI_Send(SPI_PORT, (BYTE)(arg >> 16));		/* Argument[23..16] */
	TM_SPI_Send(SPI_PORT, (BYTE)(arg >> 8));		/* Argument[15..8] */
	TM_SPI_Send(SPI_PORT, (BYTE)arg);				/* Argument[7..0] */
	n = 0x01;										/* Dummy CRC + Stop */
	if (cmd == CMD0) n = 0x95;						/* Valid CRC for CMD0(0) */
	if (cmd == CMD8) n = 0x87;						/* Valid CRC for CMD8(0x1AA) */
	TM_SPI_Send(SPI_PORT, n);

	/* Receive command resp */
	if (cmd == CMD12) {
		TM_SPI_Send(SPI_PORT, 0xFF);					/* Diacard following one byte when CMD12 */
	}
	
	n = 10;								/* Wait for response (10 bytes max) */
	do {
		res = TM_SPI_Send(SPI_PORT, 0xFF);
	} while ((res & 0x80) && --n);
...
void TM_FATFS_InitPins(void) {
	/* CS pin */
	TM_GPIO_Init(FATFS_CS_PORT, FATFS_CS_PIN, GPIO_Mode_Out_PP, GPIO_Speed_50MHz);
...
DSTATUS TM_FATFS_SD_disk_initialize (void) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) {
				ocr[n] = TM_SPI_Send(SPI_PORT, 0xFF);	/* Get 32 bit return value of R7 resp */
			}
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				...
				if (TM_DELAY_Time2() && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) {
						ocr[n] = TM_SPI_Send(SPI_PORT, 0xFF);
					}
		...
		if (TM_FATFS_SD_CardType & CT_SD2) {	/* SDC ver 2.00 */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				...
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					for (n = 64 - 16; n; n--) TM_SPI_Send(SPI_PORT, 0xFF);	/* Purge trailing data */
